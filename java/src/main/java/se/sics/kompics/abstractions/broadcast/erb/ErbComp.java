...
public class ErbComp extends ComponentDefinition {
    /** Ports **/
    private final Positive<BestEffortBroadcast> beb = requires(BestEffortBroadcast.class);
    private final Negative<EagerReliableBroadcast> erb = provides(EagerReliableBroadcast.class);

    /** Fields **/
    private Set<KompicsEvent> delivered;
    private NetAddress self;

    public ErbComp(Init init) {
        this.self = init.self;
    ...
    public static class Init extends se.sics.kompics.Init<ErbComp> {

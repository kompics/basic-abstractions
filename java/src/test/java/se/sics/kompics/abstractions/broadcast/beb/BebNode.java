...
public class BebNode extends ComponentDefinition {
    ...
    public BebNode(BebNodeInit init) {
        this.self = init.self;
        ...
        // Connections
        connect(beb.getPositive(BestEffortBroadcastPort.class), bebClient.getNegative(BestEffortBroadcastPort.class), Channel.TWO_WAY);
    ...
    public static class BebNodeInit extends se.sics.kompics.Init<BebNode> {
        private final NetAddress self;

        public BebNodeInit(NetAddress self) {
            this.self = self;
        }
    }
}
